...
extern status initMinHeap(); //creates the new heap, return OK or SYSERR. 

extern status heapinsert(pid32 pid, uint32 key); //insert a process of PID with the given key into the heap 

extern pid32 heapgethead(); //removes the root node and return its process id.

extern int32 heapminkey(); //returns the key value at the root node of the heap without removing the node.

extern pid32 heapgetitem(pid32 pid); //find and remove the process with the given pid from the heap. Status returns the process id or SYSERR
